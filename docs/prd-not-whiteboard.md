...
| 空間チャットバブル（persistent） | 空間上に永続配置されるメモ・メッセージ。議論の文脈を残す | Y.Map `spatialMessages`（永続） |
...
| レビューモード | シェイプにレビューステータス（承認/却下/コメント中）を付与 | Y.Map `reviewStatus`（永続） |
...
| 複数配置 | 空間上に複数のサブスペースを同時配置 | Y.Map `subSpaces`（永続） |
