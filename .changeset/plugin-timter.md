...
共有タイマープラグイン `usketch-plugin-timter` を追加（issue #737）。**複数タイマー**を同時に持て、**タイプ**（countdown / stopwatch）を最初から扱える拡張可能設計（`TIMER_KINDS` に足せば新タイプ追加）。2つの表現を同一モデル（`TimerCore`）で提供: (1) Controls で管理するグローバルタイマー（共有 `Y.Doc` の `timters` マップ）、(2) キャンバスに配置・移動できる**タイマーシェイプ**（`timer` シェイプ＋描画ツール、通常のシェイプ同期に乗る）。いずれも既存の Durable Object リレーで全ユーザーへ同期＋永続化（サーバの同期処理は改修不要、遅参加/リロードでも即再現）。
