...
アセットストアを追加（issue #738、tldraw `TLAssetStore` 相当）。画像などの重い blob を **content-addressed な asset レコード**として共有 `Y.Doc` の `assets` マップに一度だけ保持し、shape は `assetId` 参照のみを持つ。既存 Durable Object が任意 Y.Map を汎用リレー＆永続するため**サーバ改修なし**で全クライアント同期・遅参加/再接続でも再利用できる。
