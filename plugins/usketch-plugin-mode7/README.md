...
- `0`=**auto**（既定）＝ビューポート箱で切る。箱の外に Shape が露出せず、グリッド（`bg-grid`＝箱ぶん）と
  同じ範囲でピタッと切れる（＝地平線より上に浮かない）。
- **箱内の距離** → その手前だけ残す（遠方の散らばりを消す）。
- **箱を超える距離** → クリップを負 inset で箱の外まで広げ、**同時に `bg-grid:set-overscan` イベントで
  グリッドを箱外へ延長**する（延長px = `distance*zoom - 箱高 + 余白`）。これで grid と Shape が一緒に
  奥へ伸び、遠方 Shape の下にもグリッドが敷かれる（専用グリッド不要）。`viewport:changed` でズームにも追従。
