...
（ワールド）単位**でどこまで地面を描くか。`0`=無制限（クリップ無し・既定）、正の値でその距離より遠くを
カット＝遠くの散らばった Shape や地平線際のクラッタを消せる。実装は tilt 層ラッパへの
`clip-path: inset(top …)` で、`distance * zoom` px 分を近端に残す（ズーム変化に追従＝`viewport:changed`
で再適用）。傾きは保持（`overflow` と違い `clip-path` は 3D を潰さない）。カット線はフォグ併用でぼける。
