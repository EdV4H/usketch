...
Shape（付箋・画像・カード等、任意の既存 Shape）がグリッドのアイテムになる。各アイテムは
**自身のサイズに応じて整数セル分を占有**（大きいものは複数セルをまたぐ = ダッシュボードの
タイル）し、後続は first-fit でその周りに流し込まれる。アイテムはリサイズせず top-left だけ
スナップする。1つをドラッグして差し込むと、他のアイテムがライブでリフローする。

配置先のセル（グリッド領域）は**オーバーレイ**で可視化され、HUD の「グリッド表示」で
トグルできる。
