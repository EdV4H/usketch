// The `team-map` shape is a DATA-ONLY record (like `tilemap`): it holds the team
// registry + per-tile ownership so they persist + sync (Yjs) + undo through the
// shape store, but draws NOTHING itself — the TeamAreaLayer renders the areas.
...
export const TEAM_MAP_TYPE = "team-map";
...
/** cellKey("c,r") → teamId (sparse ownership map). */
export type OwnerMap = Record<string, string>;

export interface TeamMapShapeData extends ShapeData {
	type: "team-map";
	tile: number;
...
export function isTeamMap(shape: ShapeData): shape is TeamMapShapeData {
	return shape.type === TEAM_MAP_TYPE;
}

export function makeTeamMap(tile: number): TeamMapShapeData {
	return {
		id: generateId(),
		type: "team-map",
		x: 0,
...
export function createTeamMapShapeDefinition(tile: number = DEFAULT_TILE): ShapeDefinition {
	return {
		render: () => <g />,
		renderTarget: "svg",
		getBounds: (data): BoundingBox => {
			const d = data as TeamMapShapeData;
			return ownerBounds(d.owner ?? {}, d.tile ?? tile);
		},
		hitTest: () => false,
		resizable: false,
		resize: (data): ShapeData => data,
		createDefault: (params): ShapeData => ({ ...makeTeamMap(tile), id: params.id }),
		move: (data, dx, dy): Partial<ShapeData> => {
			...
			if (dc === 0 && dr === 0) return {};
			const next: OwnerMap = {};
			for (const [key, teamId] of Object.entries(d.owner)) {
				const [c, r] = parseCellKey(key);
				next[cellKey(c + dc, r + dr)] = teamId;
			}
		...
		serializeForAi: (data): Record<string, unknown> => {
			...
			return {
				kind: "team-map",
				teamCount: Object.keys(d.teams ?? {}).length,
