...
describe("base icon (radius tier + override)", () => {
	it("derives the settlement tier from radius (tent → town → castle)", () => {
		...
		expect(baseIconFor(8)).toBe("tent");
		expect(baseIconFor(9)).toBe("town");
		expect(baseIconFor(16)).toBe("town");
		expect(baseIconFor(17)).toBe("castle");
	...
	it("uses the radius tier when a base has no override", () => {
		...
		expect(effectiveBaseIcon({ radius: 12 })).toBe("town");
		expect(effectiveBaseIcon({ radius: 30 })).toBe("castle");
	});
