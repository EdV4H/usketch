...
describe("base icon (radius tier + override)", () => {
	it("derives the settlement tier from radius (tent → town → castle)", () => {
		...
		expect(baseIconFor(2)).toBe("tent");
		expect(baseIconFor(3)).toBe("town");
		expect(baseIconFor(5)).toBe("town");
		expect(baseIconFor(6)).toBe("castle");
	...
	it("uses the radius tier when a base has no override", () => {
		...
		expect(effectiveBaseIcon({ radius: 4 })).toBe("town");
		expect(effectiveBaseIcon({ radius: 12 })).toBe("castle");
	});
