...
- **`@edv4h/usketch-plugin-tool-openui`** — toolbar tool / side-panel prompt UI / 選択に対して実行する「Make Real」Action (Control HUD の AI グループ) / LLM provider (OpenAI 直接 / OpenAI 互換 endpoint) を提供。
...
任意の shape (フリーハンドスケッチ / 付箋の山 / 矩形でラフ書きしたワイヤフレーム等) を選択し、Control HUD (バッククォート `` ` `` で開く) の **AI** グループにある **✨ Make Real** を実行 (追従ボタンは廃止し Action に統合)。プラグインは:
