---
title: サードパーティプラグインの作成
description: 外部の npm パッケージとして uSketch プラグインを作り、ホストアプリに組み込む。
order: 22
---
...
`createApp` は非同期（`Promise<AppInstance>` を返す）で、`store` は必須。await できる async コンテキスト内で呼び出す。
