...
| `hoveredShapeId` | The shape currently under the cursor as `string \| null` (set by the select tool). Use it to adapt the hover indicator per shape type — the hover counterpart to `selection`. |
