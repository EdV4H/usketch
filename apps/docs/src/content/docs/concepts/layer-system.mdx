...
| `interactable` | If `true`, this layer receives pointer events. |
...
| Order | Layer | Source |
|-------|-------|--------|
| 0 | Base (canvas background color) | Core |
| 10 | Background (grid, dots) | Plugin |
| 50 | Shapes (main content) | Core |
| 70 | UI (selection handles, resize grips) | Plugin |
| 80 | Guides (snap lines, alignment) | Plugin |
| 90 | Transient (cursors, effects) | Core |
