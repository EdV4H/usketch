...
export class WhiteboardPage {
   ...
   * 図形を描画
   */
  async drawShape(shapeType: 'rectangle' | 'circle' | 'line', startX: number, startY: number, endX: number, endY: number) {
   ...
   * 図形をリサイズ
   */
  async resizeShape(shapeIndex: number, handle: 'nw' | 'ne' | 'sw' | 'se', deltaX: number, deltaY: number) {
   ...
   * 図形の数を取得
   */
  async getShapeCount(shapeType?: string): Promise<number> {
