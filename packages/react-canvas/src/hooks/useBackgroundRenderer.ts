...
export type BackgroundType = "none" | "dots" | "grid" | "lines" | "isometric";
...
export function useBackgroundRenderer(camera: Camera, options?: BackgroundConfig) {
	...
	const rendererRef = useRef<
		DotsRenderer | GridRenderer | LinesRenderer | IsometricRenderer | NoneRenderer | null
	>(null);
	...
	useEffect(() => {
		...
		const rendererType = rendererRef.current?.constructor.name
			.toLowerCase()
			.replace("renderer", "");
		if (!rendererRef.current || rendererType !== config.type) {
			switch (config.type) {
				case "none":
